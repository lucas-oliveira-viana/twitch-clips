...
const URLClipsLast24HoursByGame = `https://api.twitch.tv/kraken/clips/top?
    period=${PARAMETERS.period}&
    trending=true&limit=${PARAMETERS.limit}&
    game=${PARAMETERS.game}&
    language=pt-br`;

async function download() {
  const clips = (
    await axios.get(URLClipsLast24HoursByGame, {
      headers: {
        "Client-ID": CLIENT_ID,
  ...
  const videosURLS = clips.map((clip) =>
    clip.thumbnails.medium.replace("-preview-480x272.jpg", ".mp4")
  );
